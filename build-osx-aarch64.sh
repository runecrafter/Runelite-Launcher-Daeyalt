...
cp target/filtered-resources/Info.plist native-osx-aarch64/Daeyalt.app/Contents

echo Setting world execute permissions on Daeyalt
pushd native-osx-aarch64/Daeyalt.app
chmod g+x,o+x Contents/MacOS/Daeyalt
popd

codesign -f -s "${SIGNING_IDENTITY}" --entitlements osx/signing.entitlements --options runtime native-osx-aarch64/Daeyalt.app || true

# create-dmg exits with an error code due to no code signing, but is still okay
create-dmg native-osx-aarch64/Daeyalt.app native-osx-aarch64/ || true

mv native-osx-aarch64/Daeyalt\ *.dmg native-osx-aarch64/Daeyalt-aarch64.dmg

# Notarize app
if xcrun notarytool submit native-osx-aarch64/Daeyalt-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
    xcrun stapler staple native-osx-aarch64/Daeyalt-aarch64.dmg
fi
